...
else if ($w == 'u')
{
    ...
    // 이메일중복체크
    $sql = " select mb_id, mb_name, mb_nick, mb_email from {$g5['member_table']} where mb_email != '' and mb_email = '{$_POST['mb_email']}' and mb_id <> '$mb_id' ";
    $row = sql_fetch($sql);
    if ($row['mb_id'])
        alert('이미 존재하는 이메일입니다.\\nＩＤ : '.$row['mb_id'].'\\n이름 : '.$row['mb_name'].'\\n닉네임 : '.$row['mb_nick'].'\\n메일 : '.$row['mb_email']);
	...
	$ori_mb_status = $mb['mb_status'];
	$new_mb_status = $_POST['mb_status'];

	if ($ori_mb_status != $new_mb_status) {
		if ($new_mb_status === "0") {
			$sql_mb_status = " , mb_intercept_date='', mb_leave_date='' ";
		}
		if ($new_mb_status === "-1") {
			$sql_mb_status = " , mb_intercept_date='" . date("Ymd") . "' ";
		}
		if ($new_mb_status === "-2") {
			$sql_mb_status = " , mb_level='1', mb_leave_date='" . date("Ymd") . "' ";
		}
	}
