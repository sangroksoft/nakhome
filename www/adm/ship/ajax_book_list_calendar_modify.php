...
$sc_idx = $_POST['sc_idx'];
$sc_idx =  (int)preg_replace('/[^0-9]/', '', $sc_idx);
if(!$sc_idx || $sc_idx < 1)
{
	$errorstr = "예약일 키값 오류입니다.";
	$errorurl = "";
	$returnVal = returnErrorArr($errorstr,$errorurl); echo $returnVal; exit; 
}
...
for ($i=0; $i<count($_POST['chk']); $i++)
{
    ...
    // 이석호 추가 20181018
	if(!($new_bk_status == "-2" || $new_bk_status == "-1" || $new_bk_status == "0" || $new_bk_status == "1")) {
        $errorstr = "예약상태 오류항목이 있습니다.".$new_bk_status;
        $errcnt++; break;
    }
...
for ($i=0; $i<count($_POST['chk']); $i++)
{
	...
	if($scSql != "") sql_query(" update m_schedule set $scSql where s_idx='{$s_idx}' and sc_idx = '{$sc_idx}'  ");
