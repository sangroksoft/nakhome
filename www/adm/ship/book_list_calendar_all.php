...
if(!$s_idx)
{
	$fshipFetch = sql_fetch(" select s_idx from m_ship where (1) order by regdate desc limit 1 ");
	$s_idx = $fshipFetch[s_idx];
}
if(!$s_idx || $s_idx < 1) alert("등록된 어선이 없거나 존재하지 않는 어선입니다.");
...
<div class="tbl_head01 tbl_wrap" style="padding: 18px 20px;">
	...
	<div class="btn_add01 btn_add m-float" style="margin:0 0 10px">
		<a onclick="hideCanceled()">취소건 감추기</a>
	</div>
	<div class="btn_add01 btn_add m-float" style="margin:0 0 10px 10px">
		<a onclick="showCanceled()">취소건 보이기</a>
	</div>
	<div class="btn_add01 btn_add m-float" style="margin:0 0 10px 10px;line-height: 37px">
		<span id="cancelREsult"></span>
	</div>
	<div class="btn_add01 btn_add xs-hidden" style="position: absolute;top: 70px;right: 0px;">
		<a href="./book_list_all.php" id="bo_add">표형태로 보기</a>
	</div>
	...
	<table>
		...
		<thead>
			<tr>
				<th scope="col" class="xs-hidden">날짜</th>
				<th scope="col" class="xs-hidden">예약상세</th>
			</tr>
		</thead>
		<tbody>
		<?php if($totcnt > 0) { ?>
...
$( '.next-month' ).on( 'click', function() {
	...
	document.location.href = "./book_list_calendar_all.php?s_idx="+sidx+"&ym="+ym;
});
...
function chgym() {
	...
	var sidx = "<?php echo $s_idx;?>";
	document.location.href = "./book_list_calendar_all.php?s_idx="+sidx+"&ym="+go_ym;
}
</script>
<script>
