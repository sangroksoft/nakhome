...
;(function($, window, undefined){ 
  ...
  $.Calendario.prototype = {
    ...
    _generateTemplate : function(firstRun, callback) {
      ...
      this.$el.find('.fc-emptydate').parent().css({'background':'#fff', 'cursor':'default'});
      ...
      for (var i = 0; i < 7; i++){ // this loop is for weeks (rows)
        for (var j = 0; j <= 6; j++) { // this loop is for weekdays (cells)
          var pos = this.startingDay - this.options.startIn,
              ...
              past = this.year < this.today.getFullYear() || 
						this.month < this.today.getMonth() && this.year == this.today.getFullYear() ||
						this.month == this.today.getMonth() && this.year == this.today.getFullYear() && day < this.today.getDate(),
              content = '';
			  // fc_future, fc_past 가 제대로 안나오는 문제 : 위 today, past 변수에서 '==='를 '=='로 수정	
			  //alert("this.month : "+this.month+", this.today.getMonth() : "+this.today.getMonth()+", this.year : "+this.year+", this.today.getFullYear() : "+this.today.getFullYear()+"past : "+past);
          ...
          if (day <= monthLength && (i > 0 || j >= p)){
			...
			var strdate = (this.month + 1 < 10 ? '0' + (this.month + 1) : this.month + 1) + '-' + (day < 10 ? '0' + day : day) + '-' + this.year, 
				weekname_lsh = this.options.weekabbrs[j + this.options.startIn > 6 ? j + this.options.startIn - 6 - 1 : j + this.options.startIn],
				strdate_lsh = this.year+(this.month + 1 < 10 ? '0' + (this.month + 1) : this.month + 1)+(day < 10 ? '0' + day : day),
		  ...
		  if(inner == "") strdate_lsh=""; else strdate_lsh = ' id="'+strdate_lsh+'" data-wday="'+weekname_lsh+'"';  
          if(content !== '') cellClasses += 'fc-content';
          
          html += '<div class="'+cellClasses.trim()+'"'+strdate_lsh+'>' + inner + '</div>';
        }
