        ...
        while ($row2 = sql_fetch_array($result2)) {
            ...
            if(preg_match("/\.({$config['cf_image_extension']})$/i", $row2['bf_file'])) {
               // delete_board_thumbnail($bo_table, $row2['bf_file']);
                delete_board_thumbnail_lsh($bo_table, $row2['bf_file']);
            }
        }

        // 에디터 썸네일 삭제
        //delete_editor_thumbnail($row['wr_content']);
		delete_editor_img_lsh($row['wr_content']);
...
// 최근게시물 삭제2
sql_query(" delete from m_board where bo_table = '$bo_table' and wr_parent = '{$write['wr_id']}' ");
