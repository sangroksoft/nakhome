...
if ($w == 'c') // 댓글 입력
{
    ...
    */
	...
	$wr_gubun = "c";
	$wr_comt_subject = "[원글제목] ".$wr_subject;
	$wr_comt_comment = "[나의댓글] ".$wr_content;
	$sqlpost = "   insert into m_board
						set mb_id = '{$member['mb_id']}',
							 ...
							 wr_id = '{$comment_id}', 
							 wr_parent = '{$wr_id}', 
							 wr_subject = '{$wr_comt_subject}',
							 wr_comment = '{$wr_comt_comment}',
							 wr_datetime = '".G5_TIME_YMDHIS."' ";
    sql_query($sqlpost);
...
else if ($w == 'cu') // 댓글 수정
{
	...
	$sqlpost = "   update m_board
						set wr_comment = '{$wr_comt_comment}',
							 wr_datetime = '".G5_TIME_YMDHIS."' 
						where mb_id = '{$member[mb_id]}' and bo_table = '{$bo_table}' and wr_parent = '{$wr_id}' and wr_id = '{$comment_id}'	 ";
    sql_query($sqlpost);
}
