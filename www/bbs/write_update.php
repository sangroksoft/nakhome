...
$wr_device = '';
if (isset($_POST['dev_mod'])) {
    $wr_device = trim($_POST['dev_mod']);
	if(!$wr_device || !($wr_device == "p" || $wr_device == "m")) alert_gotourl_iframe("작성디바이스값 오류입니다.",G5_URL);
}

$wr_yturl    = isset($_POST['wr_yturl'])  ? trim($_POST['wr_yturl'])    : "";
$wr_yturl    = clean_xss_tags($wr_yturl);

$tags = clean_xss_tags(trim($_POST[wr_tags]));
$new_tags = "";
$errcnt = 0;
if($tags != "")
{
	$tagsArr = explode(",",$tags);
	$tagscnt = count($tagsArr);
	
	if($tagscnt > 5) $loopcnt = 5;
	else $loopcnt = $tagscnt;

	for($i=0;$i<$loopcnt;$i++)
	{
		$tagname = trim($tagsArr[$i]);
		if($tagname !="")
		{
			if (mb_strlen($tagname) > 20)  {$errcnt++; $errstr="태그는 1개당 한글 20자, 영문 40자 이하의 단어로 입력가능합니다."; break;}
			...
			$tagname = preg_replace($tag_pattern, $tag_replace, $tagname);

			if($new_tags == "") $new_tags .= $tagname;
			else $new_tags .= ",".$tagname;
		}
	}
}
if($errcnt > 0) alert($errstr);
...
if ($w == '' || $w == 'r') {
	...
	{
		// 인기태그입력
		$newtagsArr = explode(",",$new_tags);
		$newtagscnt = count($newtagsArr);
		
		for($i=0;$i<$newtagscnt;$i++)
		{
			$newtagname = $newtagsArr[$i];
			$sqlp = " insert into {$g5['popular_table']} set pp_word = '{$newtagname}', pp_date = '".G5_TIME_YMD."', pp_ip = '{$_SERVER['REMOTE_ADDR']}', pp_gubun='{$g}', pp_pidx='{$p_idx}' ";
			sql_query($sqlp, FALSE);
		}
	}
	*/
    ...
    */
    ...
    // 포스트새글 업데이트
	$sqlpost = "   update m_board
						set wr_subject = '{$wr_subject}',
							 wr_datetime = '".G5_TIME_YMDHIS."'
						where mb_id = '{$member[mb_id]}' and bo_table = '{$bo_table}' and wr_id = '{$wr_id}' ";
    sql_query($sqlpost);

	
	// 기존인기태그삭제
	/*
	$sqlpd = " delete from g5_popular where pp_pidx = '{$p_idx}'  ";
	sql_query($sqlpd);
...
$thumb_w = 480; //썸네일 가로사이즈
$thumb_h = 320; //썸네일 세로사이즈
$thumb_w2 = 780; //썸네일2 가로사이즈
$thumb_h2 = 585; //썸네일2 세로사이즈

$bo_upload_size = 5123840;

if(($w=="" && $_POST['dev_mod'] == "p") || ($w=="u" && $write['wr_device'] == "p")) // pc모드인 경우 즉, 에디터로 이미지 첨부 -- 업로드시 가로 800 사이즈로 조절하여 이미 업로드 됨.
{
	// 첫번째 이미지만 뽑아서 썸네일 만들기 - 4:3 (780 x 585)
	//$thumb = array("src"=>$src, "tgt"=>$tgt, "width"=>$thumb_width, "height"=>$thumb_height);
	$thumb = get_list_thumbnail_editor($bo_table, $wr_id, $thumb_w, $thumb_h, "thumb");
	$thumb2 = get_list_thumbnail_editor($bo_table, $wr_id, $thumb_w2, $thumb_h2, "thumb2");

    $sqlthumb = " update $write_table set wr_thumb = '{$thumb['tgt']}', wr_thumb2 = '{$thumb2['tgt']}'  where wr_id = '{$wr_id}' ";
    sql_query($sqlthumb);
}
...
@mkdir(G5_DATA_PATH.'/file/'.$bo_table.'/thumb', G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH.'/file/'.$bo_table.'/thumb', G5_DIR_PERMISSION);
@mkdir(G5_DATA_PATH.'/file/'.$bo_table.'/thumb2', G5_DIR_PERMISSION);
@chmod(G5_DATA_PATH.'/file/'.$bo_table.'/thumb2', G5_DIR_PERMISSION);
...
for ($i=0; $i<count($_FILES['bf_file']['name']); $i++) {
	...
	if (is_uploaded_file($tmp_file)) {
		...
		{
			if ($timg['2'] < 1 || $timg['2'] > 16) { $is_img = "n"; 	continue;	} //이미지 아님
			else  { $is_img = "y";}//이미지임	
		}
		//=================================================================
		...
		$dest_file_path = G5_DATA_PATH.'/file/'.$bo_table;
		$dest_file = G5_DATA_PATH.'/file/'.$bo_table.'/'.$upload[$i]['file'];
		$dest_thumb_path = G5_DATA_PATH.'/file/'.$bo_table.'/thumb';
		$dest_thumb_file = G5_DATA_PATH.'/file/'.$bo_table.'/thumb/'.$upload[$i]['file'];
		$dest_thumb_path2 = G5_DATA_PATH.'/file/'.$bo_table.'/thumb2';
		$dest_thumb_file2 = G5_DATA_PATH.'/file/'.$bo_table.'/thumb2/'.$upload[$i]['file'];

		// 업로드가 안된다면 에러메세지 출력하고 죽어버립니다.
		$error_code = move_uploaded_file($tmp_file, $dest_file) or die($_FILES['bf_file']['error'][$i]);

		//업로드된 파일이 이미지라면 썸네일 생성
		if($is_img == "y")
		{
			if($timg['0'] > 1200)
			{
...
{
	alert_replace_iframe('저장되었습니다.', G5_HTTP_BBS_URL.'/board.php?bo_table='.$bo_table.'&amp;wr_id='.$wr_id.$qstr);
}
?>
