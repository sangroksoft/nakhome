...
function thumbnail($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
	...
	// ==================== 이석호 수정  ========================
	if($source_path == $target_path)	$thumb_file = "$target_path/thumb-{$thumb_filename}_{$thumb_width}x{$thumb_height}.".$ext[$size[2]];
	else	$thumb_file = "$target_path/{$thumb_filename}.".$ext[$size[2]];
	// ==================== 이석호 수정  ========================
    ...
    $is_large = true; // 원본파일이 썸네일설정사이즈보다 큰가?
    // width, height 설정
    if($thumb_width) {
        if(!$thumb_height) {
            $thumb_height = round(($thumb_width * $size[1]) / $size[0]);
...
// ============================ 이석호 추가 ==============================
// 에디터 첫이미지 썸네일 생성
function get_list_thumbnail_editor_ori($bo_table, $wr_id, $thumb_width, $thumb_height, $is_create=false, $is_crop=true, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5, $config;
    $filename = $alt = "";
    ...
    if($tname) 
	{
		...
		$ori = G5_URL.$data_path;
		// 썸네일 이미지
		$srctmp = str_replace($filename, $tname, $data_path);
		$src = G5_URL.str_replace($filename, $tname, $data_path);
    } 
	else 
	{
    ...
    $thumb = array("src"=>$src, "ori"=>$ori, "alt"=>$alt, "filename"=>$srctmp, "filepath"=>$filepath, "width"=>$thumb_width, "height"=>$thumb_height);

    return $thumb;
}

// 에디터 첫이미지 썸네일 생성
function get_list_thumbnail_editor($bo_table, $wr_id, $thumb_width, $thumb_height, $thumb_prefix)
{
    global $g5, $config;
    $filename = $alt = "";
	...
	for($i=0; $i<count($matches[1]); $i++)
	{
		...
		if(preg_match("/\.({$config['cf_image_extension']})$/i", $srcfile) && is_file($srcfile)) 
		{
			...
			$targetname = $thumb_prefix."-".$srcname;
			$targetpath = dirname($srcfile);
	...
	// 썸네일생성
	center_crop($srcfile,$targetfile,$thumb_width,$thumb_height);
	// 오리지날 이미지 URL
	$src = G5_URL.$data_path;
	// 썸네일 이미지 URL
	$tgt = G5_URL.str_replace($srcname, $targetname, $data_path);
...
// 이미지 사이즈 조정
function imgrescale($filename, $source_path, $target_path, $thumb_width, $thumb_height, $is_create, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5;
